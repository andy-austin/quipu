...
from brain.dependencies import ALGORITHM, JWT_SECRET, verify_user
...
async def test_valid_token_returns_user_id(valid_token):
    ...
    user_id = await verify_user(creds)  # type: ignore[arg-type]
...
async def test_invalid_token_raises_401():
    ...
    with pytest.raises(HTTPException) as exc_info:
        await verify_user(creds)  # type: ignore[arg-type]
